...
export default function ArchivePage({ params }: { params: { category: string } }) {
  const [activeTab, setActiveTab] = useState(params.category)
  const [items, setItems] = useState(activeTab === 'products' ? products : research)
                ...
                >
                  <Link href={`/${activeTab}/${item.id}`} className="block h-full">
                    <div 
                      ...
                      style={{ background: item.gradient || 'linear-gradient(to bottom right, #4a5568, #2d3748)' }}
                    />
