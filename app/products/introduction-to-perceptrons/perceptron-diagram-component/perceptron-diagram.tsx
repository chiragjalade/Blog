...
export function PerceptronDiagram({ initialInputs = 3, className = '' }: PerceptronDiagramProps) {
  ...
  const [initialInputCount] = useState(initialInputs) // Added state for initial input count
  const [longPressedNode, setLongPressedNode] = useState<string | null>(null)
  const longPressTimerRef = useRef<NodeJS.Timeout>()
  const MAX_INPUTS = 7 // +1 for bias node = 8 total
  ...
  // Calculate dynamic container height based on number of input nodes
  const getContainerHeight = () => {
    const baseHeight = 300 // Base height for 3 inputs
    const additionalHeight = Math.max(0, inputCount - 3) * 50 // 50px per additional input
  ...
  const handleLongPressStart = (nodeId: string) => {
    // Prevent long press on protected nodes (bias, x1, x2, and last node xₘ)
  ...
  // Update lines on window resize or content change
  useEffect(() => {
    const updateLines = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (longPressedNode && !target.closest('.node-container')) {
        setLongPressedNode(null)
      }
    }
  ...
  const handleRevert = () => {
    if (inputCount > 3 && !isReverting) {
      ...
      const nodesToRemove = inputNodes
        .slice(3) // Start from x3
        .map(node => node.id);

      // Staggered animation for line removal
      ...
      // Start removing nodes after lines are removed
      setTimeout(() => {
        ...
        const interval = setInterval(() => {
          setInputCount((prev) => {
            if (prev <= 3) {
              ...
              // Reset removal states after animation completes
              setTimeout(() => {
                setRemovingLines([]);
                setRemovingNodes([]);
  ...
  return (
    <div
      ref={(node) => {
        // Store the DOM node in containerRef
    ...
    >
      {/* Add Input Button */}
      <div className="absolute top-4 -right-2 sm:right-4 flex items-center"> {/* Updated positioning class */}
        <AnimatePresence>
      ...
      {/* Revert Button */}
      <div className="absolute top-16 -right-2 sm:right-4 flex items-center"> {/* Updated positioning class */}
        <AnimatePresence>
      ...
      {/* SVG layer for lines and pulses */}
      <AnimatePresence>
        {inView && (
          <motion.svg
          ...
          >
            ...
            {lines.map(({ path, id, length }, index) => {
              if (id.includes('-sum') && !id.includes('bias')) {
                const pathElement = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                pathElement.setAttribute('d', path);
                const point = pathElement.getPointAtLength(length * 0.4); // Position at 40% of the line
